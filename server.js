...
function startServer() {
  ...
  fs.readFile('data_embedded.json', 'utf8', (err, data) => {
    if (err) {
      console.error('Error reading data_embedded.json:', err);
      process.exit(1);
    }
    let jsonData;
    try {
      ...
      console.error('Failed to parse data_embedded.json. The file may be empty or malformed.');
      process.exit(1);
    }
    ...
    if (
      ...
      !jsonData[0] ||
      !Array.isArray(jsonData[0].embedding) ||
      jsonData[0].embedding.length !== 1536
    ) {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      if (!Array.isArray(jsonData[i].embedding) || jsonData[i].embedding.length !== 1536) {
    ...
    app.post('/api/semantic-search', async (req, res) => {
      ...
      try {
        ...
        const scored = jsonData.map((item) => ({
          ...item,
